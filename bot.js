  ...
  fastcsv.parseStream(readStream, { headers: false })
    ...
    .on('end', () => {
      ...
      for (let i = 0; i < parts; i++) {
        ...
        fs.writeFileSync(`${vcfPath},-${i + 1}.vcf`, partContacts.join('\n'));
      }
      callback(parts);
    });
}
        ...
        convertTxtToVcf(txtPath, `${vcfPath},${fileNumber}`, namaKontak, contactCount, (parts) => {
          const sendFile = (part) => {
            return new Promise((resolve, reject) => {
              const newFileName = `${vcfPath},${fileNumber},-${part}`;
              bot.sendDocument(msg.chat.id, `${newFileName}.vcf`, {}, { contentType: 'text/x-vcard' })
                .then(() => resolve())
